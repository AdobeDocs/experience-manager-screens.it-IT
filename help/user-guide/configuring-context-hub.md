...
Prima di avviare la configurazione di un archivio dati per la configurazione delle configurazioni Context Hub per un progetto AEM Screens, è necessario configurare Google Sheets (a scopo dimostrativo).
...
Segui i passaggi indicati di seguito per configurare un archivio dati che consenta di utilizzare le configurazioni ContextHub e il percorso dei segmenti per il canale AEM Screens.
